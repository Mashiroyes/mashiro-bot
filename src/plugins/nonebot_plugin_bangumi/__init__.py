...
async def get_search_input(state):
    search_input_base = str(state["search_input"])
    if search_input_base not in ["0","1","2","3","4","5","6"]:
        if search_input_base == "退出":
            await bgm.finish(Message('已退出搜索~'))
        await bgm.finish(Message('输入内容不是编号，已退出搜索'))
    ...
    search_sort, search_type = state["search_sort"], state["search_type"]
