...
from .utils import At, MsgText, banSb, change_s_title, fi, log_fi, sd, Reply, log_sd, Reply2
...
async def _(bot: Bot, matcher: Matcher, event: GroupMessageEvent):
    ...
    try:
        ...
        time = int(''.join(map(str, list(map(lambda x: int(x), filter(lambda x: x.isdigit(), msg))))))*60
        # 提取消息中所有数字作为禁言时间
    except ValueError:
        time = None
    sb = At(event.json())
